...
class bpmn_fitness_2(base_ff):
    ...
    def all_tasks_included(self, tokens):
        ...
        unique_task_tokens = len(task_tokens) - len(set(task_tokens))

        task_numbers_ok = map(lambda tk: int(re.search(r"task\(([0-9]+)\)", tk).group(1)) < self.reader.number_of_tasks,
                              set(task_tokens))

        if all(task_numbers_ok) and unique_task_tokens == 0:
            return 0
        else:
            return -((unique_task_tokens + 1) + (sum(list(map(lambda o: not o, task_numbers_ok))) + 1))
    ...
    def are_gotos_correct(self, tokens, compiler):
        ...
        task_gotos_correctness = list(map(lambda tk: int(re.search(r"\(task([0-9]+)\)", tk).group(
            1)) < self.reader.number_of_tasks and tk not in compiler.par_tasks and tk in compiler.graph, task_gotos))
    ...
    def fitness(self, simple_graph, compiler, max_trace):
        ...
        self.check_traces([("start", 0)], [], [], (trace, i), simple_graph, compiler, max_trace)
    ...
    def check_trace(self, curr, replayed_trace, path, trace, simple_graph, compiler, max_trace):
        for nxt in bpmn_fitness.get_all_nexts(curr, simple_graph):
            self.make_a_step(curr, replayed_trace, path, trace, simple_graph, nxt, compiler, max_trace)
